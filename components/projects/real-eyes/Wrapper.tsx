    ...
    content: errorState.brokenTabs.has(tab.id) ? (
      <div className="flex items-center justify-center h-64 text-center">
        <div>
          ...
          <p className="text-sm text-gray-600 mb-4">This tab has been isolated due to errors</p>
          <button
        ...
        <div className="absolute inset-0 bg-red-900 bg-opacity-20 border border-red-500 flex items-center justify-center">
          <div className="text-center p-8">
            ...
            <p className="text-xs text-gray-500 mb-6">
              Error Count: {errorState.errorCount} | Last Error:{' '}
      ...
      <div id={id} className={`relative ${className}`} style={style} ref={containerRef}>
        ...
        {(errorState.brokenTabs.size > 0 || errorState.brokenImages.size > 0) && (
          <div className="absolute top-4 right-4 bg-yellow-600 text-white px-3 py-1 rounded text-xs z-20">
            {errorState.brokenTabs.size} tabs, {errorState.brokenImages.size} images isolated
          </div>
        )}
      </div>
