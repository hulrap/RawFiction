...
export const useEmbeddedLoading = (config: EmbeddedLoadingConfig) => {
  ...
  const {
    url,
    title,
    timeout = 25000, // AI research platforms may need more time
    maxRetries = 2, // Conservative for academic sites
    preloadDelay = 300, // Slightly longer delay for stability
    ...
    return (
      <div className="absolute inset-0 bg-black bg-opacity-90 flex items-center justify-center z-10">
        <div className="text-center p-6 max-w-md">
          ...
          <h3 className="text-lg font-semibold text-white mb-2">AI Alignment Space Unavailable</h3>
          <p className="text-sm text-gray-300 mb-4">{state.errorMessage}</p>
          <p className="text-xs text-gray-400 mb-6">
            The AI safety research platform may be experiencing high traffic or maintenance.
          </p>
          {onRetry && (
            <button
              onClick={onRetry}
      ...
      <div className="absolute inset-0 bg-black bg-opacity-75 flex items-center justify-center z-10">
        <div className="text-center">
          ...
          <h3 className="text-lg font-semibold text-white mb-2">
            Connecting to AI Safety Platform
          </h3>
          ...
          <p className="text-xs text-gray-500 mt-2">Loading AI alignment research platform...</p>
        </div>
      </div>
    );
  }
