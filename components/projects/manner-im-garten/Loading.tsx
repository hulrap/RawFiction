...
export const ContentLoadingIndicator: React.FC<{
  ...
  const getLoadingIcon = () => {
    switch (type) {
      case 'tab':
        return '🌱'; // Growing seedling for tab content
      case 'gallery':
        return '🌻'; // Sunflower for gallery images
      default:
        return '🌿'; // Herb for general loading
    }
  };
  ...
  const getGardenMessage = () => {
    ...
    switch (type) {
      case 'tab':
        return 'Tending garden content...';
      case 'gallery':
        return 'Gathering garden images...';
      default:
        return 'Nurturing community space...';
    }
  };

  return (
    <div className="absolute inset-0 bg-black bg-opacity-60 flex items-center justify-center z-10">
      <div className="text-center bg-gradient-to-br from-green-900 to-green-800 border border-green-600 rounded-lg p-6 shadow-xl">
        <div className="text-3xl mb-3 animate-pulse">{getLoadingIcon()}</div>
        <div className="text-green-100 text-sm font-medium mb-3">{getGardenMessage()}</div>
        <div className="w-40 bg-green-800 rounded-full h-2 overflow-hidden">
          <div
            className="bg-gradient-to-r from-green-400 to-green-300 h-2 rounded-full transition-all duration-500 ease-out"
            style={{ width: `${progress}%` }}
          />
        </div>
        <div className="text-xs text-green-300 mt-2 font-medium">{progress}% grown</div>
      </div>
    </div>
  );
};
